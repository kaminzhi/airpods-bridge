...
pub const HANDSHAKE: &[u8] = &[0x00, 0x00, 0x04, 0x00, 0x01, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00];
...
pub fn build_anc_payload(next_mode: u8, seq: u8, opcode: u8) -> Vec<u8> {
    vec![
        0x04, 0x00, 0x04, 0x00, 0x0a, 0x00, 
        opcode, next_mode, seq, 0x01,
...
pub fn parse_data(data: &[u8], s: &mut AirPodsState) -> bool {
    ...
    if let Some(idx) = data.windows(BATT_PREFIX.len()).position(|w| w == BATT_PREFIX) {
        if let Some(sub) = data.get(idx..) {
            if sub.len() >= 7 {
                let count = sub[6];
                let mut pos = 7;
                for _ in 0..count {
                    if pos + 4 >= sub.len() { break; }
                    ...
                    match bt {
                        0x01 | 0x04 => { s.left = info; changed = true; }
                        0x02 => { s.right = info; changed = true; }
                        0x08 => { s.case = info; changed = true; }
                        _ => {}
                    }
                    pos += 5;
                }
